...
const FREQ_LABELS = { 7: 'Hebdo', 14: 'Bi-mensuel', 30: 'Mensuel', 90: 'Trimestriel' };
const FREQ_OPTIONS = [
...
export default function Home() {
  ...
  const addTask = async () => {
    ...
    const res = await fetch('/api/tasks', {
      ...
      body: JSON.stringify({ name: name.trim(), startDate: date, frequencyDays: Number(freq) })
    });
      ...
      <main style={{ maxWidth: 620, margin: '0 auto', padding: '2.5rem 1.25rem 5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', paddingBottom: '1.5rem', borderBottom: '0.5px solid #ccc' }}>
          ...
          <button
            ...
            style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', background: 'none', border: 'none', color: showForm ? '#111' : '#888' }}
          >
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#aaa', border: '0.5px solid #e5e5e5', padding: '2px 6px' }}>
                  {FREQ_LABELS[task.frequencyDays]}
                </span>
                {/* Status */}
                {due
                  ? <span style={{ fontSize: 11, color: '#c0392b', letterSpacing: '0.04em' }}>À faire</span>
                  : <span style={{ fontSize: 11, color: '#aaa', letterSpacing: '0.04em' }}>dans {days}j</span>
                }
                {/* Delete */}
                <button
                  onClick={() => deleteTask(task.id)}
                  style={{ fontSize: 16, background: 'none', border: 'none', color: '#ccc', padding: '0 2px', lineHeight: 1 }}
        ...
        {!loading && tasks.length > 0 && (
          <div style={{ marginTop: '2rem', paddingTop: '1rem', borderTop: '0.5px solid #e5e5e5', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#aaa' }}>
              {dueCount} à faire · {tasks.length} total
            </span>
...
const btnPrimaryStyle = { fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '7px 16px', background: '#111', color: '#fff', border: 'none' };
const btnSecondaryStyle = { fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', padding: '5px 12px', background: 'none', color: '#888', border: '0.5px solid #ccc' };
